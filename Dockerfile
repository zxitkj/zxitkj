...
RUN echo 'toor' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/toor.sh

RUN echo 'cd ' >>/toor.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/toor.sh

RUN echo 'cd /noVNC-1.2.0' >>/toor.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/toor.sh

RUN chmod 755 /toor.sh
